...
As an R programmer, and a statistician, _Monday Makeovers_ were an exciting enterprise. As a 'noob', using gaming terms, I found myself excited about the idea of visualizing data in Tableau. My excitement to visualize in `R` also grew. I spent more time learning about visualizing with libraries like `ggplot2` and `lattice`. Interestingly enough, I began to ask a question: "Well, what next?". I struggled with this question for a few weeks and just moved on to the next visual. 

As I looked at visuals I, and others created, I found myself asking statistical questions."Is that relationship significant?" or "How many clusters can I confidently say exist when understanding the variances within groups?" flooded my mind. I soon realized I now had a weekly data set to not only visualize, but ask additional questions and practice statistical methods.  

With that back drop, I want to thank the folks at _Monday Makeover_ for getting the ball rolling. I am excited to see what methods I can start to apply to new visuals I get to make. 
...
There doesn't seem to be any missing data, but to cover the bases, let's use the `missmap` function from the `Amelia` library. This plot will show where missing data exists, if any.
