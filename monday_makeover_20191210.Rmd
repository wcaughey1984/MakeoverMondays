...
Monday Makeover is a weekly event in the global Tableau community I first encountered at the Tableau Conference 2019. Each week a new dataset is published. This dataset comes with a previous visual and how the data was previously displayed. Members of the Tableau community, and other communities, are encouraged to revisualize the data. These new visuals are published, shared, and commented on. It is a great way to share visualization skills and learning.

As an R programmer, and a statistician, Monday Makeovers take a different level of excitement for me. Thinking of ways to visualize data through R libraries, like `ggplot2` or `lattice`, are exciting ways to share data. What is of most interest to me is the next level of analysis. Visualizations are fun, but I am even more interested in performing statistics and analytics. My goal in these publications is to take the analysis from descriptive to analytic. This by no means suggests visualizations are not important. To the contrary, the visualizations produced inform the potential analytics which might be performed.

## Biggest Fast Food Chains in America

This weeks data set (week of 9 Dec 2019) comes from an article titled ["Ranked: Biggest Fast Food Chains in America"]("https://www.visualcapitalist.com/biggest-fast-food-chains-in-america/"). This article has a nice infographic displaying the information. This infographic presents some interest information. For instance, McDonald's is the highest ranked fast food chain by total sales (surprise, right?). One fact I caught which surprised me was the number of franchised/licensed and company units (I will just refer to them as units). I would never have thought Subway would have had more units than McDonald's. Thinking of advertisements, brand recognition, and other inputs, I would have guessed McDonald's was king of units. 

## Libraries

Before any data is brought it, let's chat about the libraries which will be used. My immediate go to libraries when I run R are `tidyverse`, `broom`, `ggplots2`, and `readr`. Using the code to pull the data, I will add `httr` and `readxl` to this list. Yes, it might seem weird to have `readr` and `readxl` both running, but we'll let it slide at this point. `knitr` will also be added to make tables a little cleaner.
...
The first major step in any analysis is data pre-processing. In this step, the data is imported, and cleaned. Part of the cleaning process will be additional feature creation.
...
Right of the bat, our friends at McDonald's are skewing the total sales figure. Since the mean is susceptible to outliers, there is a skew to the right. This can be visualized using something simple like a histogram. But first, let's make the units of measure for sales to be billions of dollars instead of dollars. This code snippet will depend upon `tidyverse` and `ggplot2` libraries. 
